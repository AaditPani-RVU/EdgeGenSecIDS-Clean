...
# === Logging Setup ===
logging.basicConfig(level=logging.INFO, format='%(asctime)s - 📡 Flow Detected: %(message)s')

# === Load Model, Scaler, Feature List, Label Encoder ===
model = torch.jit.load("ids_cnn_multiclass.pt", map_location=torch.device("cpu"))
model.eval()
...
# === Feature Extractor (Partial real-time 78D aligned) ===
def extract_flow_features(packets):
    total_pkts = len(packets)
    total_bytes = sum(len(p) for p in packets)
    durations = packets[-1].time - packets[0].time if total_pkts > 1 else 0

    sizes = [len(p) for p in packets]
    min_size = min(sizes) if sizes else 0
    max_size = max(sizes) if sizes else 0
    mean_size = sum(sizes) / total_pkts if total_pkts > 0 else 0

    fwd = [p for p in packets if IP in p and p[IP].src.startswith("192.168")]
    bwd = [p for p in packets if IP in p and p[IP].dst.startswith("192.168")]

    fwd_pkts = len(fwd)
    bwd_pkts = len(bwd)

    # Basic 8D + zero-padding to 78D
    basic_features = [
        durations,
        total_pkts,
        total_bytes,
...
if __name__ == "__main__":
    print("🚨 Real-time Multiclass IDS started...")
    print("Available interfaces:", get_if_list())

    iface = "wlan0"  # Or your actual Pi network interface
